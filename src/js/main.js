...
var containersRy = document.querySelector(".content-svg");
...
function updateInput(p, cx, cy, r1, offset, delta) {
	...
	initialValue.value = Math.round((a + 180) / .8);
}
...
function drawNeedle(cx, cy, r1, a) {
	...
	var nx1 = cx + 10 * Math.cos((a - 100) * rad);
	var ny1 = cy + 10 * Math.sin((a - 100) * rad);

	var nx2 = cx + (r1 - 50) * Math.cos(a * rad);
	var ny2 = cy + (r1 - 50) * Math.sin(a * rad);

	var nx3 = cx + 10 * Math.cos((a + 90) * rad);
	var ny3 = cy + 10 * Math.sin((a + 90) * rad);
...
function numberColor() {
	$('.typeRange').on('click', function () {
		...
		if (+number < 30) {
			...
			// getNumb.removeClass('color3');
		}
		if ((+number < 70) && (+number > 30)) {
			getNumb.attr('class', 'output');
			getNumb.addClass('color2');
			// getNumb.removeClass('color2');
			// getNumb.removeClass('color3');
		}
		if ((+number > 70)) {
			getNumb.attr('class', 'output');
...
$('.block-contact input').keypress(function(event) {
	checkParams();
